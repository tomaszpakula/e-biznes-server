    ...
    request.body.validate[Int] match {
      case JsSuccess(updatedQuantity, _) if updatedQuantity > 0 => {
        cart.find(_.productId == productId) match {
          case Some(item) => {
            cart =
              cart.map(i => if (i.productId == productId) i.copy(quantity = updatedQuantity) else i)
            Ok(
              Json.obj(
                "message" -> s"Updated quantity of item $productId from ${item.quantity} to $updatedQuantity"
              )
            )
          }
